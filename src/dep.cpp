...
bool DependencyParser::parseShiftReduce(Tree *tree) const {
  ...
  //MY CODE FOR EXTRACT DEPENDECY SCORE BETWEEN src AND dst.
  //FIRST check if src depend to next chunk
  //then score = 1
  //and push src if src doesn't depend to next chunk
  std::cout << "SCORE" << std::endl;
  for(int base = 0; base < size-2; base++)
  {
    if (estimate(tree, base, base+1, &score)) {
      hypo->head[base] = base+1;
      hypo->score[base] = score;
      // store children for dynamic_features
      hypo->children[base+1].push_back(base);
      //std::cout << "depend next" << std::endl;
      std::cout << base << " " << base+1 << " " <<"1.0000" << std::endl;
    } else {
      agenda.push(base);
      //std::cout << "not depend next" << std::endl;
    }
    //MYSIGMOID(score);
  ...
  //last-1 chunk always depend last chunk
  std::cout << size-2 << " " << size-1 << " " << "1.0000" <<std::endl;

  //calc score for pushed src to every possible dst
  int src;
  while(true){
    MYPOP(agenda,src);
    if(src == -1) break;
    for(int dst = src+2; dst < size; dst++)
    {
      estimate(tree, src, dst, &score);
      MYSIGMOID(score);
      std::cout << std::setprecision(4);
      std::cout << src << " " << dst << " " << score << std::endl;
    }
  }
  ...
  */
  ...
  for (int dst = 1; dst < size; ++dst) {
    int src = 0;
    for (;src < dst; ++ src){
      if (estimate(tree, src, dst, &score)) {
        hypo->head[src] = dst;
        hypo->score[src] = score;
        // store children for dynamic_features
        //if (!is_fake_link) {
